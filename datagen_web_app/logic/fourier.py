...
def resize_sparse_fft(sparse_mat: sp.spmatrix, new_shape: tuple[int, int], eps: float = 1e-3) -> sp.csr_matrix:
    ...
    F2_shift[r_new:r_new + low_rows, c_new:c_new + low_cols] = \
        F_shift[r_old:r_old + low_rows, c_old:c_old + low_cols]
...
def scale_sparse_matrix_fourier(original_matrix: sp.csr_matrix, new_rows: int, new_cols: int, output_path: str) -> sp.csr_matrix:
    """
    Scale a sparse matrix using Fourier transform-based resizing.
    ...
    """
    # Apply Fourier transform-based resizing
    scaled_matrix = resize_sparse_fft(original_matrix, (new_rows, new_cols))
