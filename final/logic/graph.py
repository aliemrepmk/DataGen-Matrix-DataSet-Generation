...
# ------------------------------------------------------------------
def refine_once(adj_coarse: sp.csr_matrix,
                splits: int = 2,
    ...
    n_f = n_c * splits
    fine = sp.coo_matrix((val_out, (row_out, col_out)),
                             shape=(n_f, n_f)).tocsr()
    fine.setdiag(0)
...
# ------------------------------------------------------------------
def _sample_nodes_by_strength(A: sp.csr_matrix,
                              new_size: int,
                              rng: np.random.Generator | None = None):
    """
...
# ------------------------------------------------------------------
def scale_sparse_matrix_graph(original_matrix: sp.csr_matrix, new_size: int, match_nnz: bool = True):
    """
...
# ---------- DOWN-SCALE (coarsen) ----------
    ...
    if match_nnz:
        ...
        if current_nnz > desired_nnz:
            ...
            A = sp.coo_matrix((A.data[keep], (A.row[keep], A.col[keep])), shape=A.shape).tocsr()
            ...
            A_extra = sp.coo_matrix((vals, (rows, cols)),
                                        shape=A.shape)
            A = (A + A_extra).tocsr()
            A.setdiag(0)
            A.eliminate_zeros()
